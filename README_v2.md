...
- **Support for `dbutils.secrets`and `dbutils.notebook` commands**
    Higher compatibility with Databricks notebooks and especially `secrets` allow to hide credentials from your code.
...
- **DBFS browser with file preview**
    The DBFS browser does not use sidecar any more and allows to preview many text files (csv, sh, py, ...)
...
- **Improved Databricks example notebook download from the Databricks documentation**
    Supports *.html* notebooks and Databricks magics like `%sh`, %scala%, `%sql`, ... will be properly translated to Jupyter magics
- **Support for kernels without Spark**
    For Deep Learning no Spark Session on the remote cluster is required
