...
class Progress(object):
        ...
        while (self.running == 1):
            ...
            for j, job in jobs:
                if bars.get(j, None) is None:
                    ...
                    bars[j] = FloatProgress(value=0.0, min=0.0, max=100.0, 
                                            description='Job: % 3d Stage: % 3d' % (j, 0),
                                            bar_style='info', orientation='horizontal', style=style)
                    bars[j].add_class("db-bar")
                    labels[j] = Label(value='', description='Code:', disabled=False, 
    ...
    def stop_progressbar(self, result):
        ...
        def is_running():
            job_status = [self.tracker.getJobInfo(jobid).status 
                          for jobid in self.tracker.getJobIdsForGroup(self.job_info.group_id)]
