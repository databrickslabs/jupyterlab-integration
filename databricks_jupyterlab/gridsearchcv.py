...
class GridSearchCV():
    ...
    def __init__(self, estimator, param_grid, *args, spark=None, **kwargs):
        ...
        if is_remote():
            ...
            self.gs = spark_sklearn.GridSearchCV(spark.sparkContext,
    ...
    def log_cv(self, experiment, name, tracking_uri=None):
        ...
        if is_remote():
            if os.environ.get("DBJL_ORG", None) is None:
                display(HTML('<a href=%s/#mlflow/experiments/%s>Goto experiment</a>' % 
                            (os.environ["DBJL_HOST"], experiment_id)))
            else:
                display(HTML('<a href=%s?o=%s#mlflow/experiments/%s>Goto experiment</a>' % 
                            (os.environ["DBJL_HOST"], os.environ["DBJL_ORG"], experiment_id)))
        else:
            display(HTML('<a href=%s/#/experiments/%s>Goto experiment</a>' % 
